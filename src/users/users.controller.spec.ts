...
describe('UsersController', () => {
  ...
  beforeEach(async () => {
    fakeUsersService = {
      find: (email: string) => Promise.resolve([{ id: 1, email: email, password: '1'}] as User[]),
      findOne: (id: number) => Promise.resolve({ id, email: 'asdf@asdf.com', password: '1'} as User),
  ...
  it('getUser should return a single user with given id', async() => {
    ...
    expect(user).toEqual({"email": "asdf@asdf.com", "id": 1, "password": "1"} as User)
  })
